<html>
	...
	<?php
		...
		$id=$_GET['id'];
		$sql = "Select * from use_case_complexity WHERE serial_no='$id'";
		...
		if($_SERVER["REQUEST_METHOD"] == "POST")
		{
			...
			$uc_complexity = $_POST["uc_complexity"];
			$min = $_POST["min_value"];
			$max = $_POST["max_value"];
			$weight = $_POST["weight"];
			if ( $op==0)
				$sql ="update use_case_complexity SET uc_complexity='".$uc_complexity."', min_value='".$min."', max_value='".$max."', weight ='".$weight."' where serial_no='".$serial_no."'";
			else
				$sql ="insert into use_case_complexity(uc_complexity,min_value,max_value,weight) values('".$uc_complexity."',".$min.",".$max.",".$weight.")";
	...
	<td align="center"><strong>uc complexity</strong></td>
	...
	<td align="center"><input name="uc_complexity" type="text" id="uc_complexity" value="<?php echo $rows['uc_complexity']; ?>" size="15"></td>
